...
import { teamReadTaskApproval as readTaskApproval } from './team-ops.js';
const DEFAULT_SPARKSHELL_TAIL_LINES = 400;
function resolveRecommendedTeamDir(cwd, teamName, teamStateRoot) {
    const trimmedStateRoot = typeof teamStateRoot === 'string' ? teamStateRoot.trim() : '';
    if (!trimmedStateRoot)
        return join(resolve(cwd), '.omc', 'state', 'team', teamName);
...
function resolveRecommendedTeamPath(cwd, teamName, teamStateRoot, ...segments) {
    if (!teamName)
        return null;
    return join(resolveRecommendedTeamDir(cwd, teamName, teamStateRoot), ...segments);
}
export async function readTeamPaneStatus(config, cwd = process.cwd(), snapshot, tailLines = DEFAULT_SPARKSHELL_TAIL_LINES) {
    if (!config) {
        return {
            leader_pane_id: null,
            hud_pane_id: null,
    ...
    const taskStatusById = new Map((snapshotTasks).map((task) => [task.id, task.status]));
    const taskResultById = new Map((snapshotTasks).map((task) => [task.id, task.result ?? null]));
    const taskErrorById = new Map((snapshotTasks).map((task) => [task.id, task.error ?? null]));
    const taskVersionById = new Map((snapshotTasks).map((task) => [task.id, task.version ?? null]));
    const taskCreatedAtById = new Map((snapshotTasks).map((task) => [task.id, task.created_at ?? null]));
    const taskCompletedAtById = new Map((snapshotTasks).map((task) => [task.id, task.completed_at ?? null]));
    const taskDependsOnById = new Map((snapshotTasks).map((task) => [task.id, task.depends_on ?? task.blocked_by ?? []]));
    const taskClaimPresentById = new Map((snapshotTasks).map((task) => [task.id, task.claim != null]));
    const taskClaimOwnerById = new Map((snapshotTasks).map((task) => [task.id, task.claim?.owner ?? null]));
    const taskClaimTokenById = new Map((snapshotTasks).map((task) => [task.id, task.claim?.token ?? null]));
    const taskClaimLeaseById = new Map((snapshotTasks).map((task) => [task.id, task.claim?.leased_until ?? null]));
    const taskRequiresCodeChangeById = new Map((snapshotTasks).map((task) => [task.id, task.requires_code_change ?? null]));
    ...
    const taskDescriptionById = new Map((snapshotTasks).map((task) => [task.id, task.description]));
    ...
    const taskBlockedById = new Map((snapshotTasks).map((task) => [task.id, task.blocked_by ?? []]));
    ...
    const taskRoleById = new Map((snapshotTasks).map((task) => [task.id, task.role ?? null]));
    ...
    const taskOwnerById = new Map((snapshotTasks).map((task) => [task.id, task.owner ?? null]));
    ...
    const taskSubjectById = new Map((snapshotTasks).map((task) => [task.id, task.subject]));
...
//# sourceMappingURL=pane-status.js.map