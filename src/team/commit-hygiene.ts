...
function projectRootFromTeamStateRoot(stateRoot: string | null, cwd: string): string | null {
  if (!stateRoot) return null
  const resolvedStateRoot = resolve(cwd, stateRoot)
  const stateParentDir = dirname(resolvedStateRoot)
  if (basename(resolvedStateRoot) !== 'state') return null
  if (basename(stateParentDir) !== '.omx' && basename(stateParentDir) !== '.omc') return null
